...
export default class AssignProj extends Component {
    ...
    for(let i=0;i<count2;i++){
      for(let j=0;j<count;j++){
        if(coursearray[j]['course_id']===temparray[i]['course_id']){
          temparray[i]["coursename"] = coursearray[j]['course_name'];
        }
      }
  ...
  cardData = (res) =>{
   ...
   handlePress = async () => {
    ...
    await axios.get("http://192.168.43.137:8000/api/assignments/").then(res => {
      self.cardData(res);
    })
         ...
         {self.state.array.map(function(x,i){
          return (<Card key={i}>
            ...
            <CardItem style={{ backgroundColor: 'rgba(205,98,106,0.7)' }} header>
              <Text>{self.state.array[i]["coursename"]}</Text> 
            ...
            <Animatable.Text animation="fadeOut" iterationCount={"infinite"} direction="alternate" delay={100}>
              <Text style={{color:'red', fontSize:20 }}>Deadline :{` `}</Text>
            </Animatable.Text>
            <Text style={{ fontSize: 18}}>
                   {self.state.array[i]["deadline"]}
                </Text>
            </CardItem>
         </Card>)
         })
